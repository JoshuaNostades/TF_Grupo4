...
public class GuiaController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String pasoParam = request.getParameter("paso");
        int paso = 1;

        try {
            paso = Integer.parseInt(pasoParam);
        ...
        if (paso > 4) {
            paso = 1;
            request.setAttribute("mensaje", "¡Guía completada! Ya sabes cómo funciona el flujo del sistema.");
        }

        request.setAttribute("paso", paso);
        RequestDispatcher rd = request.getRequestDispatcher("/gui/frmGuiaST.jsp");
